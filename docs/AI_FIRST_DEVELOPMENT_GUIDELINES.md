...
**Date**: 2025-01-09  
**Updated**: 2025-11-09 (Added field-selector limitation)  
**Problem**: Query "delete pods not running" → LLM generated `--field-selector status.phase!=Running`  
**Reality**: "Not running" means ANY phase except Running (Failed, Succeeded, Unknown, Pending, etc.)  
**Impact**: Command silently failed - field-selector doesn't support `!=` operator!  

**Root Cause**: LLM made TWO mistakes:
1. Semantic assumption without understanding full requirement
...
2. **Field-selector API limitation**
   - Used `--field-selector=status.phase!=Running` 
   - **Kubernetes field-selector only supports `=` (equals), NOT `!=` (not equals)**
   - Invalid selector was silently ignored → no pods matched → nothing deleted!

**Real User Case** (2025-11-09):
```bash
# User query: "delete pods not running"
$ kubectl get pods -n nm-cal-observability
NAME                                READY   STATUS                       RESTARTS   AGE
grafana-operator-7894f4648f-5jrc2   0/1     CreateContainerConfigError   0          3h21m

# LLM generated (WRONG):
$ kubectl delete $(kubectl get pods --field-selector=status.phase!=Running -o json | jq '.items[].metadata.name')
# Result: Silent failure - invalid field-selector, no pods matched, nothing deleted

# Pods still exist after "successful" execution!
```
...
**What LLM Did** (WRONG - 2025-11-09 actual output):
```bash
# ❌ Used invalid field-selector operator (silently fails!)
kubectl get pods --field-selector=status.phase!=Running -o json
kubectl delete $(kubectl get pods --field-selector=status.phase!=Running -o json | jq '.items[].metadata.name')
# Result: NO pods matched (invalid selector ignored), NOTHING deleted
```

**What LLM Should Do** (CORRECT):
```bash
# ✅ Option 1: Get all pods → filter by READY column (most reliable)
kubectl get pods -n nm-cal-observability | grep -v "1/1" | awk 'NR>1 {print $1}' | xargs kubectl delete pod -n nm-cal-observability

# ✅ Option 2: Use jq to filter by containerStatuses.ready == false
kubectl get pods -n nm-cal-observability -o json | \
  jq -r '.items[] | select(.status.containerStatuses[]?.ready == false) | .metadata.name' | \
  xargs kubectl delete pod -n nm-cal-observability

# ✅ Option 3: Multiple field-selectors with OR logic (verbose but works)
kubectl delete pods --field-selector 'status.phase=Failed' -n nm-cal-observability
kubectl delete pods --field-selector 'status.phase=Pending' -n nm-cal-observability
kubectl delete pods --field-selector 'status.phase=Unknown' -n nm-cal-observability
```
...
**AI-First Solution**: Don't hardcode examples. Instead, add **SEMANTIC UNDERSTANDING + API CONSTRAINTS** to prompt:
```python
prompt = """
...
**KUBERNETES API CONSTRAINTS:**
- field-selector only supports = (equals), NOT != (not equals)
- To match "not running", use one of:
  1. Get all pods → filter by READY column (grep -v "1/1")
  2. Get all pods JSON → jq filter by containerStatuses.ready == false
  3. Multiple field-selectors (status.phase=Failed OR status.phase=Pending)

**RECOMMENDED APPROACH for "not running":**
1. kubectl get pods (see full status with READY and STATUS columns)
2. Filter by READY != desired (e.g., grep -v "1/1") 
3. Extract pod names → kubectl delete
"""
```
